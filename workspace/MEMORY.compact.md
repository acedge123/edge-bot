...
- Cost policy: use `gpt-5.6-luna` for ordinary questions and transformations; reserve `gpt-5.6-sol` for code, production debugging, architecture, security, and high-impact work.
- Google Workspace access is provided by the workspace-local `gmail-sa` skill using the `edge@thegig.agency` service user. Inspect the installed skill instead of relying on historical memory.
